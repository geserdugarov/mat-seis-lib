...
% angAzim     - [k, 1] azimuth angles in radians
% angIncid    - [k, 1] incident angles in radians
...
%%
function coef = RTcoefModifForInverse(CijR0, CijT0, densR, densT, angAzim, angIncid, typeORpolar, indForOut)
%% Settings 
...
coef = zeros(1,numel(angAzim));
for angID = 1:numel(angAzim)
    angAz = angAzim(angID);
    angIn = angIncid(angID);
    n = [sin(angIn).*cos(angAz); sin(angIn).*sin(angAz); cos(angIn)];
