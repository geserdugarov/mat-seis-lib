...
%%
function coef = RTcoefModifForInverse(CijR0, CijT0, densR, densT, nIn, g0, typeORpolar, indForOut)
%% Settings 
...
coef = zeros(1,size(nIn,2));
for normID = 1:size(nIn,2)
    tic
    n = nIn(:,normID);
    ...
    toc
    tic
    solT1 = double(vpasolve(det(christT - eye(3)) == 0, p3));
    toc
    
    tic
    Vp0 = 5683;   Vs0 = 2367.1;
    epsilon = 0.0423;   delta =  -0.1445;   gamma = 0.253012;
    fi = 60; fi = fi*pi/180;
    a = [cos(fi) sin(fi) 0];
    coefPoly_p3 = RTcoef_polyCoef_p3([Vp0,Vs0,epsilon,gamma,delta],a,p0(1));
    solT2 = roots(coefPoly_p3);
    toc
    ...
    if max(abs(imag(solR))) < 1.0e-8
        ...
        if printErrors
            fprintf('RTcoefModifForInverse: There are complex values after searching slowness vectors (R).\n');
        end
    end
    if max(abs(imag(solT))) < 1.0e-8
        ...
        if printErrors
            fprintf('RTcoefModifForInverse: There are complex values after searching slowness vectors (T).\n');
        end
    end
    signSolR = sign(solR);
