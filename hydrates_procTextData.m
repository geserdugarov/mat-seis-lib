...
% path = 'C:\Geser\Dropbox\Projects\Газогидраты - интеграционный\компактная акустика\2018.11.20 - измерения в ИЯФ\20181120\';
% path = 'd:\Geser\experimental_data\20200317 - акустика в ИЯФ\';
path = '\\ipgg\project\Проект 14-17\ИЯФ\202003 Март\Acoustic_data\';
folds = {'20200317','20200318','20200319','20200320'};
% ID of the sample
ID = '20200318';

gain = 20; % amplification for traces plotting
plotTraceNum = 100;

numLines = 10053; % number of lines in text files
skipLines = 3;   % number of lines that should be skipped
step = 10;        % step for skipping files (1 - without skip)
sett = struct(...
    'intWind',  0.1e-6, ... % s, window length for interpolatation and finding max
    'polyfitdegr', 3, ...      % degree of interpolation polynom
    'polyfitdisc', 1.0e-8, ... % interpolation discretization
...
GasPres = [];
tracenum = 0;
for foldID = 1:numel(folds)
    fold = folds{foldID};
    F = dir([path,'\',fold,'\*_PS_*.txt']); %Выбор данных по идентификатору
    fileID = 1:step:length(F);
    tracenumTemp = numel(fileID);
    valuenum = numLines-skipLines;
    TimeTemp  = nan(valuenum,tracenumTemp);
    DataPtemp = nan(valuenum,tracenumTemp);
    DataStemp = nan(valuenum,tracenumTemp);
    TemperTemp  = nan(tracenumTemp,1);
    GasPresTemp  = nan(tracenumTemp,1);
    count = 1;
    for i = fileID
        fname = F(i).name;
        fileID = fopen([path,'\',fold,'\',fname],'r');
        temp = fgetl(fileID);
        fclose(fileID);
        temp = strsplit(temp,'\t');
        TemperTemp(count) = str2num(temp{3});
        GasPresTemp(count) = str2num(temp{5});
        temp = importdata([path,'\',fold,'\',fname],'\t',skipLines);
        temp = temp.data;
        len_tmp = size(temp,1);
        TimeTemp(1:len_tmp,count)  = temp(1:len_tmp,1);
        DataPtemp(1:len_tmp,count) = temp(1:len_tmp,3);
        DataStemp(1:len_tmp,count) = temp(1:len_tmp,6);
        count = count + 1;
        
        DateString = char(strcat(fname(1:4),'-',fname(5:6),'-',fname(7:8),{' '},fname(10:11),':',fname(12:13),':',fname(14:15)));
        formatIn ='yyyy-mm-dd HH:MM:SS';
        Global_Time(Glob_i,1) = datenum(DateString,formatIn);
        Glob_i = Glob_i + 1;
    end
    Time  = [Time  TimeTemp];
    DataP = [DataP DataPtemp];
    DataS = [DataS DataStemp];
    Temper = [Temper; TemperTemp];
    GasPres = [GasPres; GasPresTemp];
    tracenum = tracenum + tracenumTemp;
end
...
for wavetype = 1:2
    %% Load manual picks
    if wavetype < 2
        pickspath = ['data\fbs_hydrate_exp\manualPick_procTextData_',ID,'_P.dat'];
        Data = DataP;
        waveID = 'P';
    else
        pickspath = ['data\fbs_hydrate_exp\manualPick_procTextData_',ID,'_S.dat'];
        Data = DataS;
        waveID = 'S';
    end
