...
... 'timewind', [5.0e-6, 15.0e-6], ...
    ...
    'plotSkipNum', 0 ... % number of traces thar should be skipped
...
for identif = 1:size(identifList,1)
    ...
    if ~exist('reprTracesP','var')==1
        reprTracesP = zeros(valuenum,size(identifList,1)*2);
        reprTracesS = zeros(valuenum,size(identifList,1)*2);
        reprTimeP = zeros(valuenum,size(identifList,1)*2);
        reprTimeS = zeros(valuenum,size(identifList,1)*2);
        reprMaxPos = zeros(size(identifList,1)*2,4);
        reprVpVs = zeros(size(identifList,1),2*2);
        reprQpQs = zeros(size(identifList,1),2*2);
        reprDataMIT = nan(9,size(identifList,1)*2);
    end
    ...
    for wavetype = 1:2
        ...
        if wavetype==1
            ...
            if reprTraces(1,1)~= 0
                ...
                reprVpVs(identif,[1 3]) = Velocities(reprTraces(1,1)-2,:);
                reprQpQs(identif,[1 3]) = Attenuation(reprTraces(1,1)-2,:);
            ...
            if reprTraces(1,2)~= 0
                ...
                reprVpVs(identif,[2 4]) = Velocities(reprTraces(1,2)-2,:);
                reprQpQs(identif,[2 4]) = Attenuation(reprTraces(1,2)-2,:);
...
save([foldpath,'water_repr_traces.mat'],'reprTimeP','reprTracesP','reprTimeS','reprTracesS','reprMaxPos','reprDataMIT','reprVpVs','reprQpQs');
