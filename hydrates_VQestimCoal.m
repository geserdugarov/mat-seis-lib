...
datadir = 'd:\Dropbox\Projects\Matlab\data\'; % path to directory with data

%% Settings for V estimation
settV = struct(...
    'firstVal', 0.45, ...    % search first settV.firstVal*max
    'maxWind',  4.0e-6, ... % s, window after range*max
    'intWind',  1.0e-6, ... % s, window length for interpolatation and finding max
    'maxWindF2F',  2.0e-6, ... % s, window after range*max for F2F and Al
    'intWindF2F',  0.5e-6, ... % s, window length for interpolatation and finding max for F2F and Al
    'polyfitdegr', 4, ...      % degree of interpolation polynom
...
settQ = struct( ...
    ...
    'freqRangeP', [0.15e6, 0.55e6], ... % Hz, frequency range for P wave %[0.2e6, 0.5e6]
...
... 'timewind', [5.0e-6, 15.0e-6], ...
    ...
    'numOf0CrossS', 4, ... % number of zero crossing after max position for S %4
...
settSavePlot = struct( ...
    ...
    'plotReprTr', true ... % true - plot representative traces from traceList
    );
...
%     'C02' [840] [];
%     'C03' [] [];
%     'C04' [] [11:14 71 410:412 626:631];
%     'C05' [] [];
%     'C05check' [] [];
%     'C05ph2' [3251:3271] [3251:3271];
%     'C05ph4' [] [];
%     'C06' [] [];
%     'C07' [] [];
%     'C201' [1700:1859] [1700:1859];
%     'C202' [] [];
%     'C203' [] [];
%     'C204' [] [];
%     'C204ph2' [] [];
...
for identif = 1:size(identifList,1)
    ...
    ylabel(hAx(1),'external pressure, atm'); ylabel(hAx(2),'gas pressure, atm');
    title(['Sample: ',sampleID]);
    legend('P_{axial}','P_{lateral}','P_{gas}','Location','northwest');
    grid on;
    ax2 = subplot('Position',[0.05 0.36 0.90 0.25]); hold on;
    plot(3:tracenum,sampdata.DataMIT(2,:),'k');
    xlim([3,tracenum]);
    ylabel('temperature, {}^{o}C');
    legend('T_{inside camera}','Location','northwest');
    grid on;
        ...
        for trace = 1:tracenum
            ...
            if isnan(temptime)
                ...
                if trace > 2
                    ...
                    cutSig = cutSignal([Time(:,timeID) Data(:,trace)], ...
                                       [Time(rangepos,timeID),Time(rangepos,timeID)+settV.maxWindF2F]);
                end
            ...
            if trace > 2
                ...
                cutSig = cutSignal([Time(:,timeID) Data(:,trace)], ...
                                   [temptime-settV.intWind/2,temptime+settV.intWindF2F/2]);
            end
    ...
    ylabel('velocity, m/s');
    title(['Sample: ',sampleID]); 
    legend('V_P','V_S','Location','northwest');
    grid on;
    ax2 = subplot('Position',[0.05 0.36 0.90 0.25]); hold on;
    ...
    xlim([3,tracenum]);
    ylabel('attenuation');
    legend('Q_P^{-1}','Q_S^{-1}','Location','northwest');
    grid on;
    ax3 = subplot('Position',[0.05 0.06 0.90 0.25]);
    [hAx,~,~] = plotyy(3:tracenum,sampdata.DataMIT(2,:),3:tracenum,sampdata.DataMIT(3,:));
    hold(hAx(2)); plot(hAx(2),3:tracenum,sampdata.DataMIT(4,:));
    temp = [sampdata.DataMIT(3,:) sampdata.DataMIT(4,:)];
    ...
    xlim(hAx(1),[3,tracenum]); xlim(hAx(2),[3,tracenum]);
    ylabel(hAx(1),'temperature, {}^{o}C'); ylabel(hAx(2),'gas pressure, atm');
    legend('T_{inside}','P_{gas1}','P_{gas2}','Location','northwest');
    xlabel('trace number');
    grid on;
...
ax1 = subplot('Position',[0.05 0.66 0.90 0.30]); hold on;
plot(3:size(Velocities,1)+2,Velocities(:,1),'b');
plot(3:size(Velocities,1)+2,Velocities(:,2),'r');
plot(temp,Velocities(temp-2,1),'xk');
plot(temp,Velocities(temp-2,2),'xk');
legend('V_{P}','V_{S}');
hold off; grid on; ylabel('velocity, m/s');
ax2 = subplot('Position',[0.05 0.36 0.90 0.25]); hold on;
plot(3:numel(sampdata.DataMIT(2,:))+2,sampdata.DataMIT(2,:),'g');
plot(temp,sampdata.DataMIT(2,temp-2),'xk');
legend('T');
hold off; grid on; ylabel('temperature, {}^{o}C');
xlabel('trace number');
tempP = Velocities(temp-2,1);
tempS = Velocities(temp-2,2);
tempP = tempP(2:end)-tempP(1:end-1);
tempS = tempS(2:end)-tempS(1:end-1);
subplot('Position',[0.05 0.06 0.90 0.25]); hold on;
fitP = fit([1:numel(tempP)]',tempP,'exp1');
fitS = fit([1:numel(tempS)]',tempS,'exp1');
plot(1:numel(tempP),tempP,'xb');
plot(1:0.1:round(numel(tempP)*1.5),fitP(1:0.1:round(numel(tempP)*1.5)),'b');
plot(1:numel(tempS),tempS,'xr');
plot(1:0.1:round(numel(tempS)*1.5),fitS(1:0.1:round(numel(tempS)*1.5)),'r');
legend('V_{P}','V_{P} approx.','V_{S}','V_{S} approx.');
hold off; grid on; ylabel('velocity change, m/s');
xlabel('check number'); xlim([1,round(numel(tempS)*1.5)]);
linkaxes([ax1,ax2],'x');

clear ax ax1 ax2 fig;
