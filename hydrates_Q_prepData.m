...
datadir = 'd:\Dropbox\Temp\hydrates\'; % path to directory with data

%% Settings for V estimation
settV = struct(...
    'useWind',     true, ...
    'searchWind',  [0.5e-5 1.0e-4], ...
...
settQ = struct( ...
    ...
    'freqRangeP', [0.15e6, 0.8e6], ... % Hz, frequency range for P wave %[0.2e6, 0.5e6]
    'freqRangeS', [0.15e6, 0.55e6], ... % Hz, frequency range for S wave %[0.2e6, 0.5e6]
...
... 'timewind', [5.0e-6, 15.0e-6], ...
    ...
    'plotSkipNum', 200 ... % number of traces thar should be skipped
...
settSavePlot = struct( ...
    ...
    'plotTraces',   0, ... % 0 or 1 only! 1 - plot traces (used in figID)
    ...
    'maxSmpShow', 4 ... % maximum number of samples without closing windows after
    );
...
%     'S201ph2' [] [];
%     'S201ph2' [] [];
%     'CH001' [4415 4416] []; % very large dataset
%     'CH002' [] [];
%     'CH003' [3:1000] [3:1000];
%     'CH004' [] [];
    'CH005' [] [82];
...
if size(identifList,1) > settSavePlot.maxSmpShow
    fprintf(['WARNING: The number of samples > ',num2str(settSavePlot.maxSmpShow), ...
        '. Figures would be automatically closed after each sample processing.\n']);
end
...
for identif = 1:size(identifList,1)
    ...
    if ~exist('reprTracesP','var')==1
        reprTracesP = zeros(valuenum,size(identifList,1));
        reprTracesS = zeros(valuenum,size(identifList,1));
        reprTimeP = zeros(valuenum,size(identifList,1));
        reprTimeS = zeros(valuenum,size(identifList,1));
        reprMaxPos = zeros(size(identifList,1),4);
        reprVpVs = zeros(size(identifList,1),2);
        reprQpQs = zeros(size(identifList,1),2);
        reprDataMIT = nan(9,size(identifList,1));
    end
    ...
    ylabel(hAx(1),'external pressure, atm'); ylabel(hAx(2),'pore pressure, atm');
    ...
    [~,posF2F] = max(pressures(heights < 2)); % choose f2f with max pressure
    if isempty(posF2F)
        fprintf('Please check etalon data. Heights for first traces > 2 mm.\n'); 
        error('>>> STOP');
    end
    ...
    for wavetype = 1:2
        ...
        for trace = 1:tracenum
            ...
            if isnan(temptime)
                if settV.useWind % search in a window
                    time = Time( Time(:,timeID)>settV.searchWind(1) & Time(:,timeID)<settV.searchWind(2), timeID);
                    data = Data( Time(:,timeID)>settV.searchWind(1) & Time(:,timeID)<settV.searchWind(2) ,trace);
                    posWind = sum(Time(:,timeID) <= settV.searchWind(1));
                ...
                temptime = Time(posWind+rangepos+pos-1,timeID);
                maxpos(trace,3) = 0;
            end
        ...
        clear timeID badIDs findval val posWind rangepos pos temptime inttime intpoly trace ws cutSig;
        ...
        if wavetype==1
            ...
            if reprTraces(1,1)~= 0
                ...
                if reprTraces(1,1)>2
                    reprVpVs(identif,:) = Velocities(reprTraces(1,1)-2,:);
                    reprQpQs(identif,:) = Attenuation(reprTraces(1,1)-2,:);
                    reprDataMIT(:,identif) = sampdata.DataMIT(1:9,reprTraces(1,1)-2);
                else
                    reprVpVs(identif,:) = 0;
                    reprQpQs(identif,:) = 0;
                    reprDataMIT(:,identif) = zeros(9,1);
                end
...
%     plot(3:tracenum,Velocities(:,2),'r');
    ...
    %% Save velocities and attenuation in sample data struct
%     VpVsQpQs = [Velocities'; Attenuation'];
%     save(strcat(datadir,sampname),'VpVsQpQs','-append');
%     clear VpVsQpQs;
...
return
%% Saving of representative traces
reprSh = [nan; 0; 0.398; 0.507; 0.430; 0.543; 0.337; 0.161; 0.108; 0.209; 0.408; 0.570; 0.690; 0.282; 0.369; ...
    0.317; 0.786; 0.256; 0.271; 0.270; 0.332; 0.319; 0.397];
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprTracesT.dat'],reprTimeP,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprTracesP.dat'],reprTracesP,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprTracesS.dat'],reprTracesS,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprSh.dat'],reprSh,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprVpVs.dat'],reprVpVs,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprQpQs.dat'],reprQpQs,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprMaxPos.dat'],reprMaxPos,'\t');
dlmwrite([thisFolderName,'\outputs\save_for_Q\reprDataMIT.dat'],reprDataMIT,'\t');
save([thisFolderName,'\outputs\save_for_Q\data_reprTraces.mat'],'reprTimeP','reprTracesP','reprTracesS','reprMaxPos','reprDataMIT','reprVpVs','reprQpQs');

FigReprData = figure;
set(FigReprData,'units','normalized','outerposition',[0.05 0.05 0.9 0.9]);
ax1 = subplot('Position',[0.05 0.08 0.90 0.40]); hold on;
plot(reprSh,reprVpVs(:,1),'bo');
plot(reprSh,reprVpVs(:,2),'ro');
xlim([0,1]);
xlabel('hydrate saturation'); ylabel('velocity, m/s');
legend('V_P','V_S');
grid on;
ax2 = subplot('Position',[0.05 0.58 0.90 0.40]); hold on;
plot(reprSh,reprQpQs(:,1),'bo');
plot(reprSh,reprQpQs(:,2),'ro');
xlim([0,1]);
xlabel('hydrate saturation'); ylabel('attenuation');
legend('Q_P^{-1}','Q_S^{-1}');
grid on;
linkaxes([ax1,ax2],'x');
set(FigReprData,'PaperPositionMode','auto');
saveas(FigReprData,[thisFolderName,'\outputs\save_for_Q\VQdata'],'png');

identifList{1,1} = 'Al';
identifList{2,1} = 'NoHydr';

%% Plotting representative traces
for wavetype = 1:2
    FigReprTrace = figure;
    set(FigReprTrace,'units','normalized','outerposition',[0.05 0.05 0.9 0.9]);
    subplot('Position',[0.10 0.07 0.87 0.88]); hold on;
    if wavetype==1
        [~,pos] = sort(reprSh);
        pos = [1; pos(1:end-1)]; % Al data is first
        Time = reprTimeP(:,pos);
        Data = reprTracesP(:,pos);
        labelsX = {};
        for i=1:size(identifList,1)
            labelsX{i} = [identifList{pos(i),1},', Sh = ',num2str(round(reprSh(pos(i)),2))];
        end
    else
        Time = reprTimeS(:,pos);
        Data = reprTracesS(:,pos);
    end
    maxval = repmat(max(max(abs(Data))),1,size(Data,2));
    tempData = repmat(1:size(Data,2),size(Data,1),1) + Data./repmat(maxval,size(Data,1),1)/2;
    plot(Time,tempData,'k');
    tempData = (1:size(reprMaxPos,1))' + reprMaxPos(pos,wavetype*2)./maxval'/2;
    plot(reprMaxPos(pos,wavetype*2-1),tempData,'+r');
    clear maxval tempData;
    xlim([0,max(Time(end,:))]);
    ylim([0,24]);
    LinePlotExplorer();
    % figure settings for traces
    set(gca,'Ytick',1:size(reprMaxPos,1)); set(gca,'YtickLabel',labelsX);
    grid on; xlabel('times, s');
    if wavetype==1
        title('P waves, real amplitudes');
    else
        title('S waves, real amplitudes');
    end
    set(FigReprTrace,'PaperPositionMode','auto');
    saveas(FigReprTrace,[thisFolderName,'\outputs\save_for_Q\traces_initAmp_',num2str(wavetype)],'png');
end
for wavetype = 1:2
    FigReprTrace = figure;
    set(FigReprTrace,'units','normalized','outerposition',[0.05 0.05 0.9 0.9]);
    ...
    set(FigReprTrace,'PaperPositionMode','auto');
    saveas(FigReprTrace,[thisFolderName,'\outputs\save_for_Q\traces_normAmp_',num2str(wavetype)],'png');
end
for wavetype = 1:2
    shiftTime = 10.0e-6;
    FigReprTrace = figure;
    ...
    if wavetype==1
        Time = reprTimeP(:,pos);
        Time = Time - repmat((reprMaxPos(pos,1) - shiftTime)',size(Time,1),1);
        Data = reprTracesP(:,pos);
    else
        Time = reprTimeS(:,pos);
        Time = Time - repmat((reprMaxPos(pos,3) - shiftTime)',size(Time,1),1);
        Data = reprTracesS(:,pos);
    end
    maxval = max(abs(Data));
    tempData = repmat(1:size(Data,2),size(Data,1),1) + Data./repmat(maxval,size(Data,1),1)/2;
    plot(Time,tempData,'k');
    clear maxval tempData;
    xlim([0,100.0e-6]); ylim([0,24]);
    LinePlotExplorer();
    % figure settings for traces
    set(gca,'Ytick',1:size(reprMaxPos,1)); set(gca,'YtickLabel',labelsX);
    grid on; xlabel('times, s');
    if wavetype==1
        title('P waves, normalized amplitudes, shifted time');
    else
        title('S waves, normalized amplitudes, shifted time');
    end
    set(FigReprTrace,'PaperPositionMode','auto');
    saveas(FigReprTrace,[thisFolderName,'\outputs\save_for_Q\traces_normAmp_shiftT_',num2str(wavetype)],'png');
end
...
save([foldpath,'CH4_repr_traces.mat'],'reprTimeP','reprTracesP','reprTimeS','reprTracesS','reprMaxPos','reprDataMIT','reprVpVs','reprQpQs');
