classdef LinePlotReducer_seism < handle
...
% Geser Dugarov - add reducing for y direction
    ...
    methods
        ...
        function o = LinePlotReducer_seism(varargin)
            ...
            % Get the axes width and height once.
            width  = get_axes_width(o.h_axes);
            o.last_width  = width;
            o.last_limsX  = [-inf inf];
            o.last_limsY  = [-inf inf];

            % Reduce the data!
            xlim = get(o.h_axes, 'XLim');
            ylim = get(o.h_axes, 'YLim');
            [x_r, y_r] = reduce_to_height(o.x, o.y, ylim);
            for k = 1:length(y_r)
                [x_r{k}, y_r{k}] = reduce_to_width(x_r{k}(:), y_r{k}(:), width, xlim);
            end
            ...
            for k = 1:length(o.h_axes)
                addlistener(o.h_axes(k), 'Units', 'PreSet', ...
                            @(~,~) o.UnitsPreSet);
                addlistener(o.h_axes(k), 'YLim',  'PostSet', ...
                            @(~,~) o.RefreshData);
                addlistener(o.h_axes(k), size_cb{:}, ...
                            @(~,~) o.RefreshData);
            end
    ...
    methods
        ...
        function RefreshData(o)
            ...
            % just for plotyy.
            limsX = get(o.h_axes(1), 'XLim');
            limsY = get(o.h_axes(1), 'YLim');
            ...
            if width == o.last_width && all(limsX == o.last_limsX) && all(limsY == o.last_limsY)
                o.busy = false;
                return;
            end
            ...
            [x_r, y_r] = reduce_to_height(o.x, o.y, limsY);
            for k = 1:length(y_r)
                
                % Reduce the data.
                [x_r{k}, y_r{k}] = reduce_to_width(x_r{k}(:), y_r{k}(:), width, limsX);
                
                % Update the plot.
                set(o.h_plot(k), 'XData', x_r{k}, 'YData', y_r{k});
