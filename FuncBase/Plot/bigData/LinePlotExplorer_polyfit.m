classdef LinePlotExplorer_polyfit < handle
    
% LinePlotExplorer_polyfit
% added polyfit on right mouse click
%
...
% Geser Dugarov
% added y directions zoom and moving
    ...
    methods
        ...
        function o = LinePlotExplorer_polyfit(varargin)
            
            % Record the figure number and min and max value on the x axis.
            if nargin < 1, o.filepath = 'undefpath.dat'; else o.filepath = varargin{1}; end;
            if nargin < 2, o.h_fig =  gcf; else o.h_fig = varargin{2}; end;
            if nargin < 3, o.x_min = -inf; else o.x_min = varargin{3}; end;
            if nargin < 4, o.x_max =  inf; else o.x_max = varargin{4}; end;
            if nargin < 5, o.y_min = -inf; else o.y_min = varargin{5}; end;
            if nargin < 6, o.y_max =  inf; else o.y_max = varargin{6}; end;
    ...
    methods (Access = protected)
        ...
        function Scroll(o, h, event, varargin)
            ...
            modifiers = get(o.h_fig,'currentModifier');        %(Use an actual figure number if known)
            ...
            if ctrlIsPressed
                ...
                new_lims = max(min(new_lims, o.y_max), o.y_min);

                % Update the axes.
                set(h_axes, 'XLim', old_x_lims);
        ...
        function ButtonDown(o, h, event, varargin)
            ...
            if strcmp(type,'normal')
                ...
                fileID = fopen(o.filepath,'w');
                fprintf(fileID,'%d %1.4e\n',data');
                fclose(fileID);
            end
        ...
        function Motion(o, h, event, varargin)
            ...
            if o.button_down
                ...
                point    = get(o.button_down_axes, 'CurrentPoint');
                movementX = point(1, 1) - o.button_down_point(1);
                movementY = point(1, 2) - o.button_down_point(2);
                xlims    = get(o.button_down_axes, 'XLim');
                ylims    = get(o.button_down_axes, 'YLim');
                
                % Don't let the user pan too far.
                new_limsX = xlims - movementX;
                new_limsY = ylims - movementY;
                if new_limsX(1) < o.x_min
                    new_limsX = o.x_min + [0 diff(new_limsX)];
                end
                if new_limsX(2) > o.x_max
                    new_limsX = o.x_max - [diff(new_limsX) 0];
                end
                if new_limsY(1) < o.y_min
                    new_limsY = o.y_min + [0 diff(new_limsY)];
                end
                if new_limsY(2) > o.y_max
                    new_limsY = o.y_max - [diff(new_limsY) 0];
                end

                % Update the axes.
                set(o.button_down_axes, 'XLim', new_limsX);
                set(o.button_down_axes, 'YLim', new_limsY);
